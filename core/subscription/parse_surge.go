...
func ParseSurgeText(content string) ([]ProxyNode, error) {
	lines := extractProxyLikeLines(content, SplitSections(content)["proxy"])
	if len(lines) == 0 {
...
func extractProxyLikeLines(content string, lines []string) []string {
	if len(lines) > 0 {
		return lines
	}
	res := make([]string, 0)
...
func isProxyLikeLine(line string) bool {
	...
	switch normalizeProxyType(parts[0]) {
	case "ss", "ssr", "vmess", "vless", "trojan", "http", "https", "socks", "socks5":
		return true
	default:
		return false
	}
